...
function loadParentDropdownList(inputID = null, selectedValue = 0, selectedText = null, defaultText = null, addNewOption = null) {
    ajaxRequest({
        url: '/Dropdowns/Get', type: 'POST', data: {
            Table: 'ePharmacy.dbo.SubModule', Columns: 'ID Value, Description Text', Condition: 'WHERE IsDeleted =0'
        }, callBack: function (responseJSON) {
            bindJQueryDropdownList(responseJSON.resultJSON, $('#' + (inputID == null ? 'ParentID' : inputID)), (defaultText == null ? 'Select Parent' : defaultText), selectedValue, selectedText, addNewOption);
        }
    });
}
